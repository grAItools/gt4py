...
def _format_snippet_lines(
    location: SourceLocation, secondary: Sequence[_UnderlineRow] = ()
) -> Optional[list[str]]:
    """
    Render the source lines of ``location`` with a line-number gutter and carets.

    ``secondary`` spans (which must lie on the first line of ``location``) are
    underlined with ``-`` markers below the caret row.
    ...
    gutter_width = len(str(start_line + len(snippet_lines) - 1))
    result: list[str] = []
    for i, line in enumerate(snippet_lines):
        lineno = start_line + i
        result.append(f"{lineno:>{gutter_width}} | {line}")
        if i == 0 and location.column is not None:
            # Carets only under the first line: from the start column to either the
            # end column (single-line locations) or the end of the line.
            caret_start = location.column - 1
            if location.end_line == location.line and location.end_column is not None:
                caret_width = max(location.end_column - location.column, 1)
            else:
                caret_width = max(len(line) - caret_start, 1)
            caret_line = " " * caret_start + "^" * caret_width
            result.append(f"{'':>{gutter_width}} | {caret_line}")
            for column, end_column, sec_label in sorted(secondary, key=lambda s: s[0]):
                width = max(end_column - column, 1) if end_column is not None else 1
                underline = " " * (column - 1) + "-" * width
                result.append(f"{'':>{gutter_width}} | {underline} {sec_label}")
    if truncated:
        result.append(f"{'':>{gutter_width}} | ...")
    return result
...
def format_location(
    ...
    snippet_lines = _format_snippet_lines(loc, secondary)
    if snippet_lines is None:
        return loc_str
    if label and len(snippet_lines) >= 2:
        snippet_lines[1] = f"{snippet_lines[1]} {label}"
    snippet_str = "\n".join(snippet_lines)
